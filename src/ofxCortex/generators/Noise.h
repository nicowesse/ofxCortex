...
public:
  static double getNoise(Noise::Settings settings);
  
  static double getNoise(const glm::vec3 &sample, const ofParameterGroup &parameters);
  
  static double getNoise(glm::vec3 sample, int seed, float scale, float contrast = 1.0f, float contrastBias = 0.5f, float details = 0.5f, float roughness = 1.5f, int octaves = 3);
...
#pragma mark - Noise - Pixel/Image Methods
  ...
  static void writeNoiseToFbo(ofFbo &fbo, glm::vec3 speed, float scale, float contrast = 1.0f, float contrastBias = 0.5f, float details = 0.5f, float roughness = 1.5f, int octaves = 3, int seed = 80057);
